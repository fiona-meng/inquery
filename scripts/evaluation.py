"""
Official BIRD benchmark evaluation script.
...
def compute_acc_by_diff(exec_results, diff_json_path):
    ...
    simple_acc      = sum(r['res'] for r in simple_results)      / len(simple_results)
    moderate_acc    = sum(r['res'] for r in moderate_results)    / len(moderate_results)
    challenging_acc = sum(r['res'] for r in challenging_results) / len(challenging_results)
    all_acc         = sum(results) / len(results)
    count_lists = [len(simple_results), len(moderate_results), len(challenging_results), len(results)]
